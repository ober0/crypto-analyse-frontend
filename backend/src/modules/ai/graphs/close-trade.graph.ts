...
import { closeTradeParser, CloseTradeResultType } from "../response-schemas/close-trade";
...
const logger = new Logger("CloseTradeGraph");

function getValidToolCalls(toolCalls?: ToolCall[]) {
    return (toolCalls ?? []).filter((call) => call?.name);
}

export const CloseTradeGraphState = Annotation.Root({
    ...
    result: Annotation<CloseTradeResultType>({
        reducer: (_, next) => next
    })
});

export function createCloseTradeGraph(toolsService: ToolsService) {
    const defaultTools = [toolsService.webSearchTool, toolsService.marketDataTool, toolsService.indicatorsTool];

    return new StateGraph(CloseTradeGraphState)
        ...
        .addNode("llm", async (state, config) => {
            ...
            const nodeResult: typeof CloseTradeGraphState.Update = {
                messages: [response],
            ...
            if (!validToolCalls.length) {
                try {
                    const validateResponse = await closeTradeParser.parse(response.text);
